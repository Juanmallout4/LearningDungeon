...
- **Lectura unificada:** los endpoints `GET /api/users/:userId/evaluations` y `GET /api/clubs/:clubId/evaluations` consultan las tablas de evaluación en paralelo y fusionan el resultado en una sola lista ordenada por fecha, etiquetando cada fila con `evaluationType: 'tul' | 'category' | 'technique'`. Esto evita duplicar lógica en `ProfileScreen`/`ReportsScreen`, que solo necesitan ramificar el renderizado según ese discriminador.
- **"Pedir Técnica" (`tul_technique_requests` / `tul_technique_evaluations`):** mecanismo ligero, pensado sobre todo para Taekwondo, mediante el cual un instructor anuncia una técnica concreta del catálogo de vocabulario a todo un grupo (`POST /api/groups/:groupId/technique-requests`) y luego califica a cada alumno individualmente con una nota 1-5, un comentario y, opcionalmente, puntos de juego extra (`POST /api/technique-requests/:requestId/evaluations`, upsert por `(request_id, student_id)`). Las calificaciones resultantes se incorporan a la lectura unificada de evaluaciones como `evaluationType: 'technique'` y aparecen en el informe mensual del alumno (`ProfileScreen`, pestaña "Informes") junto con la imagen de la técnica y la insignia de puntos otorgados.
...
A nivel individual, `GET /api/users/:userId/evaluations` y `GET /api/users/:userId/attendance` aceptan también `month`/`year` opcionales: con ellos, la pestaña "Informes" de `ProfileScreen` deja de mostrar agregados de todo el histórico y pasa a ser un **informe mensual real**, navegable mes a mes, donde el alumno ve su nota media, su asistencia y el detalle de cada evaluación (incluidas las de "Pedir Técnica") correspondientes únicamente al mes seleccionado.
